...
const VALID_STATUSES = ["pending", "processing", "in_progress", "done", "cancelled"];
...
// ─── Email: commande prise en charge (processing) ────────────────────────────
function buildProcessingHtml(order: any): string {
  ...
  return `<!DOCTYPE html>
<html lang="fr">
<head><meta charset="UTF-8"><title>Commande prise en charge — Chreol Empire</title></head>
...
<div style="max-width:580px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="text-align:center;margin-bottom:24px;">
    ...
    <p style="color:#666;font-size:13px;margin:0;">Mise à jour de votre commande</p>
  </div>

  <div style="background:#0A1500;border-radius:12px;border:1px solid #3B82F644;padding:16px;margin-bottom:20px;text-align:center;">
    <p style="color:#3B82F6;font-size:15px;font-weight:900;margin:0 0 4px;">✅ Commande prise en charge</p>
    <p style="color:#93C5FD;font-size:12px;margin:0;">📅 ${dateStr}</p>
  </div>

  <div style="background:#141519;border-radius:16px;border:1px solid #2A2B30;padding:24px;margin-bottom:16px;">
    <p style="color:#FFFFFF;font-size:15px;font-weight:800;margin:0 0 12px;">Notre équipe traite votre commande</p>
    <p style="color:#AAAAAA;font-size:13px;line-height:21px;margin:0 0 16px;">
      Votre commande <strong style="color:#C9A84C;">#${shortId}</strong> a bien été reçue et est en cours de traitement par notre équipe.
      Vous recevrez un email de confirmation dès que la livraison sera effectuée.
    </p>
    <div style="background:#0A0B0F;border-radius:12px;padding:14px;">
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr><td style="color:#666;font-size:12px;padding:6px 0;">Résumé</td><td style="color:#AAA;font-size:12px;text-align:right;">${order.summary ?? "—"}</td></tr>
        <tr style="border-top:1px solid #2A2B30;">
          <td style="color:#AAA;font-size:13px;font-weight:700;padding:10px 0 0;">Total</td>
          <td style="color:#C9A84C;font-size:18px;font-weight:900;text-align:right;padding:10px 0 0;">${totalFmt} FCFA</td>
        </tr>
      </table>
    </div>
    <div style="text-align:center;margin-top:16px;">
      <p style="color:#AAAAAA;font-size:12px;margin:0 0 10px;">Une question ? Contactez-nous directement</p>
      <a href="https://wa.me/+237697657734" style="display:inline-block;background:#25D366;color:#FFFFFF;font-size:13px;font-weight:800;text-decoration:none;padding:10px 24px;border-radius:100px;">
...
function buildReceiptHtml(order: any): string {
  ...
  if (detailType === "uba_recharge") {
    headerIcon = "🏦"; headerTitle = "Reçu de Recharge UBA";
    headerSubtitle = "UBA Cameroun — Recharge de compte"; totalLabel = "Montant rechargé";
    detailRows = `
    ...
    headerIcon = "💳"; headerTitle = "Confirmation UBA";
    detailRows = `
      <tr><td style="color:#666;font-size:12px;padding:8px 0;">Titulaire</td><td style="color:#FFF;text-align:right;">${details.name ?? "—"}</td></tr>
      <tr><td style="color:#666;font-size:12px;padding:8px 0;">Téléphone</td><td style="color:#FFF;text-align:right;">${details.phone ?? "—"}</td></tr>`;
  } else if (detailType === "crypto_buy") {
    headerIcon = "₿"; headerTitle = "Preuve de Livraison Crypto";
    headerSubtitle = "Transaction blockchain confirmée"; colorAccent = "#26A17B"; totalLabel = "Valeur en FCFA";
    ...
    headerIcon = "💶"; headerTitle = "Confirmation Envoi PayPal";
    headerSubtitle = "Paiement PayPal effectué"; colorAccent = "#60A5FA"; totalLabel = "Montant envoyé";
    detailRows = `
      <tr><td style="color:#666;font-size:12px;padding:8px 0;">Compte PayPal</td><td style="color:#60A5FA;text-align:right;">${details.paypalId ?? details.paypalEmail ?? "—"}</td></tr>`;
  } else if (detailType === "crypto_sell" || detailType === "paypal_sell") {
    headerIcon = "📲"; headerTitle = "Confirmation de Versement";
    headerSubtitle = "Paiement Mobile Money effectué";
    const isOrange = details.operator === "Orange" || details.operator === "Orange Money";
    colorAccent = isOrange ? "#FF6600" : "#FACC15"; totalLabel = "Montant versé";
    detailRows = `
    ...
    headerIcon = "🎟️"; headerTitle = "Bon de Livraison Coupon"; colorAccent = "#A78BFA"; totalLabel = "Montant échangé";
    const isOrange = details.operator === "Orange" || details.operator === "Orange Money";
    detailRows = `
...
<div style="max-width:580px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="background:#141519;border-radius:16px;border:1px solid #2A2B30;padding:28px;margin-bottom:16px;">
    ...
    <div style="background:#0A0B0F;border-radius:12px;padding:16px;margin-bottom:16px;">
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="color:#666;font-size:12px;padding:8px 0;">N° Commande</td><td style="color:#888;font-size:11px;font-family:monospace;text-align:right;">#${(order.id ?? "").slice(-8).toUpperCase()}</td></tr>
        ${detailRows}
        ${order.payment_method ? `<tr><td style="color:#666;font-size:12px;padding:8px 0;">Mode de paiement</td><td style="color:#FFF;text-align:right;">💳 ${order.payment_method}</td></tr>` : ""}
        <tr style="border-top:1px solid #2A2B30;">
  ...
  <div style="background:#0A1200;border-radius:16px;border:1px solid #25D36633;padding:18px;margin-bottom:16px;text-align:center;">
    ...
    <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
      <a href="https://www.google.com/maps/search/?api=1&query=Chreol+Empire+Douala+Cameroun" style="display:inline-block;background:#4285F4;color:#FFFFFF;font-size:13px;font-weight:800;text-decoration:none;padding:10px 18px;border-radius:100px;margin:4px;">🗺️ Avis Google</a>
      <a href="https://www.trustpilot.com/review/monelecam.fr" style="display:inline-block;background:#00B67A;color:#FFFFFF;font-size:13px;font-weight:800;text-decoration:none;padding:10px 18px;border-radius:100px;margin:4px;">★ Avis Trustpilot</a>
    </div>
  </div>
...
serve(async (req) => {
  ...
  try {
    ...
    if (["processing", "in_progress", "done"].includes(status)) {
      try {
        const { data: order } = await supabase
          .from("orders")
          .select("id, type, summary, total, payment_method, client_name, client_email, details, gift_code, push_token")
          .eq("id", orderId)
          .single();

        if (status === "processing") {
          // Email "prise en charge" + push
          if (order?.client_email) {
            const html = buildProcessingHtml(order);
            await sendBrevo(order.client_email, order.client_name || "client", "✅ Votre commande est prise en charge — Chreol Empire", html);
          }
          await sendExpoPush(order?.push_token, "✅ Commande prise en charge", "Notre équipe traite votre commande. Vous serez notifié à chaque étape.");

        } else if (status === "in_progress") {
          // Push uniquement, pas d'email
          await sendExpoPush(order?.push_token, "⚙️ En cours de livraison", "Votre commande est en cours de livraison. Vous recevrez votre code bientôt.");
          ...
          if (order?.client_email) {
            ...
            await sendBrevo(order.client_email, order.client_name || "client", "🎉 Votre commande a été livrée — Chreol Empire", html);
          }
          const code = giftCode ?? order?.gift_code;
          await sendExpoPush(
            order?.push_token,
            "🎉 Commande livrée !",
            code ? `Votre code est disponible : ${code}` : "Votre commande a été livrée avec succès !"
          );
        }
      } catch (err) {
        console.error("[admin-update-order] action error =", String(err));
      }
    }
